(function(root, factory) {
	...
	var replaceUtils = {
		...
		tel : function (el) {
			...
			// 替换输入框获取的字符
			var str = inputTxt.replace(pattern, '')		// 清除"数字"和"."以外的字符
								.replace(/^[^1]/g, "")  // 验证第一个字符不是1
								.replace(/^(1)[^346789]/g, "$1");  // 验证第二个字符不是[3、4、5、6、7、8、9]
			// 替换输入框值
			el.value = str;
			return str;
		},
		...
		type1 : function (el) {
			// 非法输入正则
			var pattern = new RegExp(/[^0-9a-zA-Z-\_]+/g);
			// 获取输入框值
			var inputTxt = el.value;
			// 替换输入框获取的字符
			var str = inputTxt.replace(pattern, '');
	...
	inputAutoVerify.fn = inputAutoVerify.prototype = {
		...
		eachInputElement : function (elArr) {
			...
			for (; i < len; i++) {
				// 获取验证元素属性
				var curObj = this.getElementAttribute(elArr[i]);
				// 判断字段名和对象不存在
				if (!curObj || !curObj.field) {
					console.log('字段名和对象不存在!');
					return;
				}
				this.$elArr[curObj.field] = curObj;
				if (this.checkLegal(curObj, this.$options.callBack)) {
					flag = true;
				}
			}
			return flag;
		...
		getElementAttribute : function (el) {
			var obj = {};
			// 元素字段名
			obj.field = el.getAttribute("name");
			// 元素名称
		...
		getInputBoxAttribute : function (el) {
			...
			// 判断是否实时验证
			if (this.$options.checkRealTime) {
				this.checkLegal(obj, this.$options.callBack);
			}
		...
		mathInputType : function (obj, el) {
			switch (obj.rule.type) {
				case "tel":
					console.log("手机号~");
					obj.value = replaceUtils.tel(el);
					break;
				case "cardNumber":
					console.log("证件号码~");
					obj.value = replaceUtils.cardNumber(el);
					break;
				case "type1":
					console.log("字母数字下划线横线~");
					obj.value = replaceUtils.type1(el);
					break;
				default:	// 默认是不能输入非法字符
					console.log("特殊字符~");
					obj.value = replaceUtils.specialCharacters(el);
					break;
			}
		},
	};
