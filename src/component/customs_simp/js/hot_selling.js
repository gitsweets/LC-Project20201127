...
(function(root, factory) {
	...
	var data = {
			// 热卖区接口
			hotSelling : {
				reqUrl	: '2/lancare_customhouse_interface_Tianjin/api:hot_selling_area',
				reqDevUrl	: '2/lancare_customhouse_interface_Tianjin/api:hot_selling_area',
	...
	// 载入商品详情页面
	var loadHotSelling = function (apendEl, hashName) {
		// 判断要插入html位置是否存在
		if (!apendEl) {
	...
	loadHotSelling.prototype = {
		init : function (apendEl, hashName) {
			...
			// 请求预约信息
			this.reqInterfaceApi(data.hotSelling, _param,function (d) {
				console.log(d);
	...
	var init = function (obj, hashName) {
		...
		// 载入功能模块
		return new loadHotSelling(document.getElementById('customs_simp'), hashName);
	};
