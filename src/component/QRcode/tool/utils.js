(function(root, factory) {
	...
	var utils_ = {
		...
		// 把元素插入到指定元素内的最前面(此方法只适用单个操作多个循环时，顺序不对)
		insertBeforeDom : function (apendDom, dom) {
			// console.log(apendDom);
			// console.log(dom);
			// 在值传进来时，需要先配置好dom元素(即必须是元素，不能是带有length的)
			// 判断要插入的元素是否正确
			if (!this.isDomObject(apendDom) || !this.isDomObject(dom)) {
				console.log("元素或指定元素不是dom元素~");
			}
			// 获取父级元素的第一个子级元素
			var childrenDom = apendDom.children[0];
			...
			if (childrenDom) {
				console.log(2);
				// 两个方法都可以，结果一样
				// apendDom.insertBefore(dom, childrenDom);
				apendDom.insertAdjacentElement("afterbegin", dom);
			} else {
				console.log(1);
				apendDom.appendChild(dom);
			}

			// 插入元素
			// apendDom.insertBefore();
			return;
			// 判断apendEl是否存在, 没有设置默认值为 body
			if (!apendEl || (apendEl == "body")) {
				// 插入到body的最前面
				// var firstEl = document.body.firstChild;//得到页面的第一个元素
				// document.body.insertBefore(html, firstEl);
				apendDom.insertBefore(dom, document.getElementsByClassName(siblingEl)[0]);
			} else {
				// 判断插入的class是否存在
				if (document.getElementsByClassName(apendEl).length > 0) {
					document.getElementsByClassName(apendEl)[0].insertBefore(dom, document.getElementsByClassName(siblingEl)[0]);
		...
		// 把字符串插入指定元素的最前面
		insertBeforeHtmlStr : function (obj, apendEl) {
			// 判断要插入html位置是否存在
			if (!apendEl) {
				console.log("未配置模块运行id~");
				apendEl = document.body;
			}
			console.log(apendEl);
			...
			for (i in obj) {
				// 把字符串转成dom元素
				var dom = this.parseDom(obj[i])[0];
				console.log(dom);
				// 把元素插入到指定元素内的最前面
				this.insertBeforeDom(apendEl, dom);
			}
		},
